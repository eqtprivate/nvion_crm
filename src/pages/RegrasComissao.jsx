      ...
      <DialogContent className="max-w-3xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><Label>Comissão base</Label><PercentInput value={form.percentual_base || ''} onChange={(value) => setForm({ ...form, percentual_base: value })} /></div>
            <div><Label>Comissão vendedor</Label><PercentInput value={form.percentual_vendedor || ''} onChange={(value) => setForm({ ...form, percentual_vendedor: value })} /></div>
            <div><Label>Comissão líder</Label><PercentInput value={form.percentual_lider || ''} onChange={(value) => setForm({ ...form, percentual_lider: value })} /></div>
            ...
            <div><Label>Parcelas da comissão</Label><Input type="number" min="1" max="60" placeholder="Ex: 3" value={form.num_parcelas_comissao || ''} onChange={(e) => setForm({ ...form, num_parcelas_comissao: e.target.value })} /></div>
...
export default function RegrasComissao() {
  ...
  const createMutation = useMutation({ mutationFn: (data) => base44.entities.RegrasComissao.create({ ...data, empresa_vinculada: empresa }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['regrasComissao', empresa] }); setDialogOpen(false); setSelectedRegra(null); } });
  const updateMutation = useMutation({ mutationFn: ({ id, data }) => base44.entities.RegrasComissao.update(id, data), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['regrasComissao', empresa] }); setDialogOpen(false); setSelectedRegra(null); } });
  const deleteMutation = useMutation({ mutationFn: (id) => base44.entities.RegrasComissao.delete(id), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['regrasComissao', empresa] }) });
  ...
  const exportCSV = () => { const headers = ['Regra', 'Produto', 'Administradora', 'Tipo', 'Base', 'Vendedor', 'Líder', 'Prazo', 'Parcelas', 'Prazo 1ª Parcela', 'Status']; const rows = filtered.map((item) => [item.nome_regra, item.produto, item.administradora, item.tipo_comissao, item.percentual_base, item.percentual_vendedor, item.percentual_lider, item.prazo_pagamento_dias, item.num_parcelas_comissao, item.prazo_primeira_parcela_dias, item.status]); const csv = [headers, ...rows].map((row) => row.map((cell) => `"${cell || ''}"`).join(',')).join('\n'); const blob = new Blob([csv], { type: 'text/csv' }); const url = window.URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'regras_comissao.csv'; link.click(); window.URL.revokeObjectURL(url); };

  return (
    <div className="p-4 sm:p-8 bg-gray-50 min-h-screen">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4"><div><h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Regras de Comissão</h1><p className="text-gray-500 mt-1">Parâmetros comerciais por produto, administradora e perfil de comissão</p></div><div className="flex gap-2"><Button variant="outline" onClick={exportCSV} disabled={filtered.length === 0}><Download className="w-4 h-4 mr-2" />Exportar CSV</Button><Button onClick={() => { setSelectedRegra(null); setDialogOpen(true); }}><Plus className="w-4 h-4 mr-2" />Nova Regra</Button></div></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6"><Card><CardContent className="p-4"><p className="text-sm text-gray-500">Total de Regras</p><p className="text-2xl font-bold">{kpis.total}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-gray-500">Regras Ativas</p><p className="text-2xl font-bold text-green-700">{kpis.ativas}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-gray-500">Comissão Média</p><p className="text-2xl font-bold text-primary">{formatPercent(kpis.comissaoMedia)}</p></CardContent></Card><Card><CardContent className="p-4"><p className="text-sm text-gray-500">Produtos com Regra</p><p className="text-2xl font-bold text-blue-700">{kpis.produtos}</p></CardContent></Card></div>
      <div className="bg-white rounded-lg shadow"><div className="p-4 border-b"><div className="relative max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" /><Input placeholder="Buscar regras..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9" /></div></div><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Regra</TableHead><TableHead>Produto</TableHead><TableHead>Administradora</TableHead><TableHead>Tipo</TableHead><TableHead>Base</TableHead><TableHead>Vendedor</TableHead><TableHead>Líder</TableHead><TableHead>Parcelas</TableHead><TableHead>Status</TableHead><TableHead></TableHead></TableRow></TableHeader><TableBody>{isLoading ? <TableRow><TableCell colSpan={10} className="text-center py-8">Carregando regras...</TableCell></TableRow> : filtered.length === 0 ? <TableRow><TableCell colSpan={10} className="text-center py-12 text-gray-500"><Percent className="w-12 h-12 mx-auto mb-2 text-gray-300" />Nenhuma regra encontrada</TableCell></TableRow> : filtered.map((regra) => <TableRow key={regra.id}><TableCell>{regra.nome_regra}</TableCell><TableCell>{regra.produto || '-'}</TableCell><TableCell>{regra.administradora || '-'}</TableCell><TableCell>{tipoLabel[regra.tipo_comissao] || regra.tipo_comissao}</TableCell><TableCell>{formatPercent(regra.percentual_base)}</TableCell><TableCell>{formatPercent(regra.percentual_vendedor)}</TableCell><TableCell>{formatPercent(regra.percentual_lider)}</TableCell><TableCell>{regra.num_parcelas_comissao ? `${regra.num_parcelas_comissao}x` : '-'}</TableCell><TableCell><Badge>{regra.status || '-'}</Badge></TableCell><TableCell><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><MoreVertical className="w-4 h-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => { setSelectedRegra(regra); setDialogOpen(true); }}>Editar</DropdownMenuItem><DropdownMenuItem className="text-red-600" onClick={() => deleteMutation.mutate(regra.id)}>Excluir</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}</TableBody></Table></div></div>
