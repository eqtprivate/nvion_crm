...
const stageColors = {
  ...
  simulacao: 'bg-yellow-100 text-yellow-800',
  proposta_enviada: 'bg-orange-100 text-orange-800',
  documentacao: 'bg-cyan-100 text-cyan-800',
  em_aprovacao: 'bg-indigo-100 text-indigo-800',
...
export default function Oportunidades() {
  ...
  const kpis = useMemo(() => {
    ...
    const valorTotal = ganhas.reduce((sum, o) => sum + (o.valor_carta || 0), 0);
    const pipeline = oportunidades
      .filter(o => o.status === 'aberta')
      .reduce((sum, o) => sum + (o.valor_carta || 0), 0);

    return { total, abertas, ganhas: ganhas.length, valorTotal, pipeline };
  }, [oportunidades]);

  const exportToCSV = () => {
    const headers = ['Nome', 'Produto', 'Valor Carta', 'Stage', 'Status', 'Previsão Fechamento', 'Probabilidade'];
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-blue-600">{kpis.abertas}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-gray-500">Ganhas</p>
            <p className="text-2xl font-bold text-green-600">{kpis.ganhas}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-gray-500">Pipeline (R$)</p>
            <p className="text-2xl font-bold text-primary">{kpis.pipeline.toLocaleString('pt-BR')}</p>
          </CardContent>
        </Card>
      </div>
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-12 text-gray-500">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm">Crie sua primeira oportunidade acima</span>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={op.id} className="hover:bg-gray-50">
                    ...
                    <TableCell>
                      <span className="font-medium">
                        {op.valor_carta ? `R$ ${op.valor_carta.toLocaleString('pt-BR')}` : '-'}
                      </span>
                    </TableCell>
                    <TableCell>
                      <Select
                        value={op.stage || 'novo_contato'}
                        onValueChange={(value) => handleQuickUpdate(op.id, 'stage', value)}
                      >
                    ...
                    <TableCell className="hidden lg:table-cell text-sm">
                      {op.previsao_fechamento
                        ? new Date(op.previsao_fechamento).toLocaleDateString('pt-BR')
                        : '-'}
                    </TableCell>
